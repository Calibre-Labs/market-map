...
export function initDb(dbPath) {
  ...
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS sessions (
      ...
      intent_origin TEXT,
      intent_anchor TEXT,
      intent_candidate TEXT,
      intent_candidate_confidence REAL,
      intent_change_status TEXT,
      created_at INTEGER NOT NULL,
  ...
  ensureColumn("tab_id", "TEXT");
  ensureColumn("plan_text", "TEXT");
  ensureColumn("plan_questions", "TEXT");
  ensureColumn("plan_status", "TEXT");
  ensureColumn("intent_origin", "TEXT");
  ensureColumn("intent_anchor", "TEXT");
  ensureColumn("intent_candidate", "TEXT");
  ensureColumn("intent_candidate_confidence", "REAL");
  ensureColumn("intent_change_status", "TEXT");
  return db;
}
...
export function createSession(db, session) {
  const stmt = db.prepare(`
    INSERT INTO sessions (
      id, user_id, tab_id, username, status, phase, turn_count,
      chat_history, trace_json, root_span, root_span_id, root_span_span_id,
      plan_text, plan_questions, plan_status,
      intent_origin, intent_anchor, intent_candidate, intent_candidate_confidence, intent_change_status,
  ...
  stmt.run(
    ...
    session.intent_origin ?? null,
    session.intent_anchor ?? null,
    session.intent_candidate ?? null,
    session.intent_candidate_confidence ?? null,
    session.intent_change_status ?? null,
    session.created_at,
    session.updated_at
  );
...
export function getActiveSessionForUserAndTab(db, userId, tabId) {
  if (!tabId) return getActiveSessionForUser(db, userId);
  return (
    db
      .prepare(
        "SELECT * FROM sessions WHERE user_id = ? AND tab_id = ? AND status = 'active' ORDER BY created_at DESC LIMIT 1"
      )
      .get(userId, tabId) || null
  );
}

export function listStaleActiveSessions(db, updatedBefore, limit = 100) {
  return db
    .prepare(
      "SELECT * FROM sessions WHERE status = 'active' AND updated_at <= ? ORDER BY updated_at ASC LIMIT ?"
    )
    .all(updatedBefore, limit);
}
