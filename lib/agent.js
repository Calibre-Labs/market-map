...
function extractSourcesFromText(text) {
  ...
  if (leadingJson) tryParse(leadingJson);

  const broadJson = extractJsonBlock(raw);
  if (broadJson && broadJson !== leadingJson) tryParse(broadJson);
...
export function extractSources(grounding) {
  return extractGroundingSources(grounding);
}
