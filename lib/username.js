...
export function isAffirmative(message) {
  ...
  return /\b(yes|yep|yeah|correct|right|switch|change|do it|go ahead|proceed|confirm|use that)\b/.test(
    normalized
  );
}
...
export function isNegative(message) {
  const normalized = (message || "").trim().toLowerCase();
  if (!normalized) return false;
  return /\b(no|nope|nah|don\'t|do not|keep|stay|original|current|wrong|not that)\b/.test(
    normalized
  );
}
...
export function inferIntentAnchorFromHistory(chatHistory) {
  if (!Array.isArray(chatHistory)) return null;
  for (const entry of chatHistory) {
    if (entry?.role !== "user") continue;
    const content = typeof entry?.content === "string" ? entry.content.trim() : "";
    if (!content || isConfirmation(content)) continue;
    return content;
  }
  return null;
}
