# Market Map LLM Prompts + State Machine (Current)

This document is synced to the current implementation in:
- `lib/agent.js`
- `lib/fsm.js`
- `server.js`
...
### 1.1 `buildSystemInstruction(mode)` common instruction
...
### 1.4 Intent change classifier (`assessIntentChange`)
...
### 1.5 Input validity classifier (`assessInputValidity`)
...
## 2) Runtime Stage Pipeline and Data Handoff
...
### 3.1 Derived state mapping (`deriveSessionState`)
...
### 3.2 Reducer events and target states
...
### 3.3 Invariants (`validateSessionInvariants`)
- If `status=complete`:
...
## 4) Stage Output -> Next Stage Input (Quick Map)
