...
export async function repairSources({
  ...
  return traced(
    async (span) => {
      const prompt = `Provide 4 valid sources that directly support the numeric metrics in the result below.\nCategory: ${category}\n\nResult:\n${resultText}\n\nReturn JSON only in this shape:\n{\n  "sources": [\n    {"title": "...", "url": "..."}\n  ]\n}`;
...
export async function generateSourcesForResult({
  ai,
  model,
  category,
  resultText
  ...
  return traced(
    async (span) => {
      const prompt = `Find 3-5 sources that directly support the numeric metrics in the result below. Prefer primary sources (company filings, investor relations, earnings releases) or authoritative sources (G2, Gartner, Forrester) that mention the numbers.\n\nCategory: ${category}\n\nResult:\n${resultText}\n\nReturn JSON only in this shape:\n{\n  "sources": [\n    {"title": "...", "url": "..."}\n  ]\n}\n\nRules:\n- Only include sources that support the specific metrics shown.\n- No commentary or extra text.`;
