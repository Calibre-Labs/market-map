...
export function formatSourcesMarkdown(sources) {
  const parts = sources.map((source) => {
    ...
    const title = source.title && source.title !== source.url
      ? source.title
      : domain || source.url;
...
/**
 * Reject URLs that are obviously hallucinated or malformed before we
 * waste time on HTTP validation.
 */
function isPlausibleUrl(url) {
  if (typeof url !== "string") return false;
  if (url.length > 300) return false;
  // Catch repeating-pattern hallucinations like "1y4y1y4y1y4y…"
  if (/(.{2,8})\1{4,}/.test(url)) return false;
  try {
...
export async function validateSources(sources) {
  const sane = sources.filter((s) => isPlausibleUrl(s.url));
  const rejected = sources.filter((s) => !isPlausibleUrl(s.url));
  const checks = await Promise.all(
    sane.map(async (source) => ({
      source,
